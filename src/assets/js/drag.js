...
(function ($) {
    var __imgx = 0, //图片宽度
        ...
        __sliderVerifyCode = "", //验证和登录的传参
        __codediv;//验证码区域的div
    $.fn.slide = function (options) {
        var imgspec = options.imgspec;
        __successCallBack = options.successCallBack;
        __getUrl = options.getImageUrl;
        __verifiyUrl = options.verifiyUrl;
        __successCallBack = options.successCallBack;
        __completeCallBack = options.completeCallBack;
        //校验参数
    ...
    function __init() {
        ...
        CreadeCodeDiv();
        $('#drag').drag(__successCallBack, __imgx, __imgy, __codediv, __verifiyUrl);
        $.ajax({ //获取验证码
            type: "POST",
            url: __getUrl,
            dataType: "JSON",
            ...
            success: function (result) {
                //if (result['state'] === -1) {//错误次数处理
                //    return;
                //}
                $('.mask').css('zIndex', 999).css('display', 'flex')
                ...
                __img__sliderVerifyCodey = result.data.sliderVerifyCode;
                ...
                if(result.data && result.data.sliderVerifyCode){
                    sliderVerifyCode = result.data.sliderVerifyCode
                }else{
                    sliderVerifyCode = ''
                }
...
*/
(function ($) {
    $.fn.drag = function (__successCallBack, imgx, imgy, __codediv, url) {
        ...
        this.append(html);
        $(".refesh_bg").click(function () { console.log('%cVerificationCode Refresh', 'color:blue'); $('#' + __codediv).refresh(); });
        ...
        var handler = drag.find('.handler'),
            ...
            maxWidth = imgx - handler.width(), //能滑动的最大间距
            t1 = new Date(), //开始滑动时间
            t2 = new Date(); //结束滑动时间
        ...
        function dragmovend(thisx) {
            ...
            $.ajax({ //校验
                ...
                data: JSON.stringify({
                    ...
                    sliderVerifyCode: __img__sliderVerifyCodey
                }),
                success: function (result) {
                    if (result.code === 200 && result.data > 0) {
                        ...
                        if (__successCallBack !== undefined)
                            __successCallBack(sliderVerifyCode);
                        dragOk();
                    } else {
                        $(".refesh_bg").show(300);
        ...
        function dragOk() {
            ...
            $(".refesh_bg").hide();
            $('.mask').css('zIndex', -1).hide();
            __completeCallBack({sliderVerifyCode:__img__sliderVerifyCodey})
        }
