...
export default {
  ...
  computed: {
    ...mapState(['USERTYPE', 'USERNAME', 'SYSTEMID', 'NAME', 'UNIT', 'USERID', 'DROPDOWNBOX', 'buttonAuth', 'authority', 'LOGODIAPLAY', 'LOGOSWITCH', 'eventId', 'TOKEN', 'Config']),
    apis() {
      return apis
    },
    tableHeight() {
  ...
  methods: {
     ...
     * @param {rowIndex} 第几行
     */
    differBg({ row, columnIndex }) {
      if (row.flag === '1' && columnIndex === 1) {
        return {
    ...
    exportFuncForm(url) {
      ...
      Object.keys(this.searchData).forEach((key) => {
        ...
        input.type = 'text'
        input.name = key
        input.value = this.searchData[key]
    ...
    // 展开收缩公共
    expCol() {
      // this.$refs.expCol.tack()
      // debugger
      this.isOpenSearch = this.$refs.expCol.shzk
    },
    /* 删除 */
    deleteHandle(url, id) {
    ...
    chineseToPinYin(character) {
      ...
      enChar = enChar.replace(/ /g, '-')
      while (enChar.indexOf('--') > 0) {
        enChar = enChar.replace('--', '-')
      }
      return enChar
    },
    // 在对象中搜索
  ...
  mounted() {
    if (this.buttonAuth.includes('BTN_LIST')) {
