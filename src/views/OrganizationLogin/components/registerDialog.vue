...
<template>
  <el-dialog
    ...
    <el-form ref="form" :model="form" label-width="200px" :rules="rules" size="mini">
      <el-form-item :label="$t('ZYZZMC')" prop="organizationName">
        <el-input v-model="form.organizationName"></el-input>
      </el-form-item>
      <el-form-item :label="$t('FZRXM')" prop="personInCharge">
        <el-input v-model="form.personInCharge"></el-input>
      </el-form-item>
      <el-form-item :label="$t('LXDH')" prop="picMobile">
        <el-input v-model="form.picMobile"></el-input>
      </el-form-item>
      <el-form-item :label="$t('CLRQ')" prop="foundDate">
        <el-date-picker
      ...
      <el-form-item :label="$t('ZZGM')" prop="volunteerNum">
        <el-input v-model="form.volunteerNum"></el-input>
      </el-form-item>
      <el-form-item :label="$t('QQQ')" prop="qqGroup">
        <el-input v-model="form.qqGroup"></el-input>
      </el-form-item>
      <el-form-item :label="$t('YB')" prop="postCode">
        <el-input v-model="form.postCode"></el-input>
      </el-form-item>
      <el-form-item :label="$t('CZ')" prop="fax">
        <el-input v-model="form.fax"></el-input>
      </el-form-item>
      <el-form-item :label="$t('FWLY')" prop="serviceScope">
        <el-checkbox-group v-model="form.serviceScope" :max="4">
          <el-checkbox v-for="item in DROPDOWNBOX.zyz_service_position" :key="item.value" :label="item.value" :value="item.value">{{item.text}}</el-checkbox>
        </el-checkbox-group>
      </el-form-item>
      <el-form-item :label="$t('volunteerExp')" prop="serviceExperience">
        <el-input type="textarea" v-model="form.serviceExperience" :maxlength="500" show-word-limit :autosize="{ minRows: 3 }"></el-input>
      </el-form-item>
      <el-form-item :label="$t('XXDZ')" prop="address">
        <el-input v-model="form.address"></el-input>
      </el-form-item>
      <el-form-item :label="$t('SJHZH')" prop="phone">
        <el-input v-model="form.phone">
          <button slot="append" class="verifi" v-if="isShowVerifyCode" @click.prevent="handleBtn" :disabled="countTime < 60">{{countTime === 60 ? '获取验证码' : `${countTime}s`}}</button>
        </el-input>
      </el-form-item>
      ...
      <el-form-item :label="$t('SFZH')" prop="credentialNo">
      ...
      <el-form-item :label="$t('password')" prop="password">
        <el-input v-model="form.password" @input="checkData" @focus="tipVisible = true" @blur="tipVisible=false" type="password">
...
export default {
  ...
  methods: {
    init() {
     ...
     // 获取字典
    getDro() {
      this.$http.post(apis.OptionConfig_GetDictZyzServicePosition, { dictGroups: ["ZYZ_SERVICE_POSITION"] }).then((res) => {
    ...
    registeredAccount() {
      if (this.isSendVerifyCode) {
        ...
        this.$http.post(apis.Auth_ZyzOrganizationRegistration, formData).then((res) => {
        if (res.data.code === 200) {
          this.closeDialog()
        }
        })
